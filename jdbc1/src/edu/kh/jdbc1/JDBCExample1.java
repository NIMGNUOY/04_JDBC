...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// JDBC(Java DataBase Connectivity) : Java 에서 DB에 연결할 수 있게 해주는 Java Programming API
		// 																	(Java에 기본 내장된 인터페이스)
		// java.sql 패키지에서 제공
		...
		// 2. DBMS (Oracle)
		// 3. Oracle 에서 Java와 연결할 때 사용할 JDBC를 상속받아 구현할 클래스 모음(ojdbc10.jar 라이브러리)
		...
		//  (Java 와 DB사이를 연결해주는 통로)
		
		Statement stmt = null;
		// Connection 을 통해 SQL문을 DB에 전달하여 실행하고 생성된 결과(ResultSet, 성공한 행의 개수)를 
		// 반환(Java)하는데 사용되는 객체
		...
		try {
			...
			String port = ":1521";				// 포트 번호 1521 (기본값)
			
			String sid = ":XE";					// DB 이름 
			
			// url == jdbc:oracle:thin:@localhost:1521:XE
			
			String user = "kh_sym";				// 사용자계정
			...
			// 중간확인
			// System.out.println(conn);	// oracle.jdbc.driver.T4CConnection@7219ec67
			
			
			// 3. SQL 작성
			// *** JAVA 에서 작성되는 SQL은 마지막에 세미콜론(;)을 찍으면 안된다!! ***
			String sql = "SELECT EMP_ID, EMP_NAME, SALARY, HIRE_DATE FROM EMPLOYEE";
			
			// 4. Statement 객체 생성
			// -> Connection 객체를 통해서 생성
			stmt = conn.createStatement();		// StateMent 참조변수에 담아주기
			
			
			// 5. 생성된 Statement 객체에 sql 을 적재하여 실행한 후 결과를 반환받아와서
			//	rs 변수에 저장
			rs = stmt.executeQuery(sql);
			// executeQuery() : SELECT 문 수행 메서드, ResultSet 반환
			
			
			// 3단계 : SQL 을 수행해서 반환 받은 결과(ResultSet)를
			//		  한 행씩 접근해서 컬럼값 얻어오기
			
			while ( rs.next() ) {
				// rs.next() : rs 가 참조하고 있는 ResultSet 객체의 첫번째 컬럼부터
				//			  순서대로 한 행씩 이동하며 다음행이 있을 경우 true, 없으면 false 반환
				
				// rs.get자료형("컬럼명") 
				String empId = rs.getString("EMP_ID");	// "200" (첫번째 턴)
				...
				// java.sql.Date 의 toString() 은 yyyy-mm-dd 형식으로 오버라이딩 되어있음(시분초 X)
